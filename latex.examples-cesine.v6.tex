...
%Easy tree structures based off of brackting:
	%\usepackage{qtree}
	\usepackage{qqtree} %changes: the original qtree is incompatible with xyling because both use the command \Tree, so i changed qtree so that it uses \QTree instead, now qqtree and xyling are compatible
...
%Complicated tree structures and much more
	%\usepackage{xyling}
	\usepackage{xyling} %changes: i added text arrows that go over the text. not an important change, so most people can use xyling instead.
...
\printindex
%latex the file once, then run this at the commandline in the same directory: makeindex  latex.examples-cesine.v5.ist
